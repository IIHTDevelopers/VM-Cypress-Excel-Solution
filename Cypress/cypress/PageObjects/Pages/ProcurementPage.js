  ...
  verifyRequestedDateColumnDateWithinRange(from, toDate) {
    ...
    return cy
      ...
      .then(() => {
        ...
        const fromDateFormatted = dayjs(from, "DD-MM-YYYY").format("YYYY-MM-DD");
      ...
      .then(() => {
        ...
        const fromDateParsed = dayjs(from, "DD-MM-YYYY").startOf("day"); // Start of day for fromDate
        ...
        return cy
          ...
          .then(() => {
            return cy.wrap(dateElementsToVerify).each((dateStr) => {
              const requestedDateParsed = dayjs(dateStr, "YYYY-MM-DD").startOf("day");
  
              // Check if the requested date is within the specified range
              const isInRange =
                requestedDateParsed >= fromDateParsed && requestedDateParsed <= toDateParsed;
              if (!isInRange) {
